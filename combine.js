class CombineSystem {
    ...
    performMerge() {
        if (!this.canMerge()) {
            if (this.player.money < this.cost) showDialog("Not enough money! Need $5000.");
        ...
        const currentMaxMerge = Math.max(p1.mergeCount || 0, p2.mergeCount || 0, p3.mergeCount || 0);
        ...
        const sources = [...this.slots].sort((a, b) => b.sourceIndex - a.sourceIndex);
        ...
        if (this.player.team.length < 6) {
            ...
            if (!placed) showDialog("Box and Party full! (Merge failed safely)"); // Edge case
        }
