...
class Renderer {
    constructor(canvas, world, player) {
        // OPTIMIZATION: alpha: false tells browser the background is opaque (faster)
    ...
    addParticle(x, y, color = null) {
        // OPTIMIZATION: Hard cap particles to 30 to prevent lag spikes
    ...
    drawParticles(offsetX, offsetY) {
        for (let i = this.particles.length - 1; i >= 0; i--) {
            ...
            p.life -= 0.05; // Fade slightly faster
            p.x += p.vx;
            p.y += p.vy;
            ...
            // Simple rendering (no globalAlpha changes if possible)
            this.ctx.fillStyle = p.color || `rgba(200, 200, 200, ${p.life})`;
            this.ctx.fillRect(drawX, drawY, 3, 3);
        }
    }

    draw() {
        // Calculate visible range (+1 buffer for smooth scrolling)
        const cx = Math.ceil(this.canvas.width / TILE_SIZE) + 1;
        const cy = Math.ceil(this.canvas.height / TILE_SIZE) + 1;
        const startTileX = Math.floor(this.player.x) - Math.floor(cx / 2);
        const startTileY = Math.floor(this.player.y) - Math.floor(cy / 2);
        ...
        for (let y = 0; y < cy; y++) {
            for (let x = 0; x < cx; x++) {
                ...
                // 1. Draw Base Tile (With 1px Overlap to fix grid lines)
        ...
        // --- RENDER LIST SYSTEM (Depth Sorting) ---
        let renderList = [];
    ...
    drawCrop(crop) {
        ...
        this.ctx.textAlign = 'center';
        this.ctx.fillText(crop.ready ? '🍓' : '🌱', drawX + TILE_SIZE / 2, drawY + TILE_SIZE / 1.5);
    }
    ...
    drawBuilding(building) {
        ...
        if (building.type === 'pokecenter') {
            // Fake Shadow
            this.ctx.fillStyle = 'rgba(0,0,0,0.2)';
            this.ctx.fillRect(Math.floor(drawX) - 5, Math.floor(drawY) - 5, TILE_SIZE + 20, TILE_SIZE + 20);
            ...
            this.ctx.drawImage(this.houseImg, Math.floor(drawX) - TILE_SIZE * 1.5, Math.floor(drawY) - TILE_SIZE * 2, TILE_SIZE * 4, TILE_SIZE * 4);
            ...
            // Fake Shadow
            this.ctx.fillStyle = 'rgba(0,0,0,0.2)';
            this.ctx.fillRect(Math.floor(drawX) + 5, Math.floor(drawY) + 5, TILE_SIZE, TILE_SIZE);
    ...
    drawNPC(npc) {
        ...
        let bounce = t < 1 ? Math.sin(t * Math.PI) * 10 : 0;

        let drawX = (currX - this.player.x) * TILE_SIZE + this.canvas.width / 2 - TILE_SIZE / 2;
        let drawY = (currY - this.player.y) * TILE_SIZE + this.canvas.height / 2 - TILE_SIZE / 2 - bounce;

        if (npc.type === 'daycare') this.ctx.drawImage(this.daycareImg, Math.floor(drawX) - 10, Math.floor(drawY) - 10, TILE_SIZE + 20, TILE_SIZE + 20);
        else if (npc.type === 'talk' && npc.name === 'Villager') this.ctx.drawImage(this.villagerImg, Math.floor(drawX) - 10, Math.floor(drawY) - 10, TILE_SIZE + 20, TILE_SIZE + 20);
        else if (npc.type === 'quest') this.ctx.drawImage(this.herbalistImg, Math.floor(drawX) - 10, Math.floor(drawY) - 10, TILE_SIZE + 20, TILE_SIZE + 20);
        else {
    ...
    drawPlayer(player) {
        ...
        if (this.player.moving && Math.random() < 0.1) {
            this.addParticle(this.player.x + 0.5, this.player.y + 0.9);
        }

        let bounceY = this.player.moving ? Math.abs(Math.sin(Date.now() / 100)) * -5 : 0;
    ...
    drawOverlays() {
        ...
        if (typeof enemySystem !== 'undefined') {
            enemySystem.projectiles.forEach(p => {
                ...
                this.ctx.fillStyle = p.color || '#8e44ad';
                this.ctx.beginPath();
                ...
                if (defenseSystem.turretOffsets && Array.isArray(defenseSystem.defenders)) {
                    defenseSystem.turretOffsets.forEach((offset, idx) => {
                        ...
                        if (t && t.hp > 0) {
                            ...
                            this.ctx.strokeRect(tx + 15, ty + 15, TILE_SIZE - 30, TILE_SIZE - 30);
                        }
                    });
                }
